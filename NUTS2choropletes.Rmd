...
\begin{itemize}
  \item TIME designates the year of the observation.
  \item OBS is the actual numeric observaion value.
  \item FREQ tells us the observation frequency. Our data is annual (A) based on the data retrieval call.
  \item UNIT  hast been set to percentage (PC) in the data retrieval call.
  \item SEX is the total (T) for both sexes. \marginnote{Possible values for SEX are \underline{M}ale, \underline{F}emale and \underline{T}otal (not transgender as you might think).}
  \item GEO lists the geographic entities at NUTS-2 level.
\end{itemize}
...
R treats the unemployment figures as a continuous variable. To get a better feeling of the actual level of unemployment in the respective regions we can group the regions into different classes. This process is calles \emph{binning} and is essentially a transformation into a discrete variable.
...
The labelling text of the legend has also been adapted in this step. 
...
# Borders on NUTS1 level in black
...
map <- map + geom_path(data=eurMapDf_NUTS1, color='black', size=0.01)
map
```
...
A post on Max Marchis Blog[^MMarchi] was the source of the initial draft of this document. However this is just one of many. Searcing for maps, R, choropletes and Eurostat will certainly reveal numerous other sources.

The `ggplot2`[^ggdoc] package has a great online documentation with many practical examples.


Last but not least this document has been created to get used to the Tufte-\LaTeX\ [^tufte_latex] document classes which define a style similar to the style Edward Tufte uses in his books and handouts. Tufte’s style is known for its extensive use of sidenotes, tight integration of graphics with text, and well-set typography. That said, if you neet ot print it, use color as it will be hard to read otherwise.
