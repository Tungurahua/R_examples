...
\begin{itemize}
  \item merge the the main dataset \texttt{dfSdmx} with the variable dictionary `VARIABLE\_codes` doing an inner join
  \item select the variables we need to create the plot (`GEO`, `VARIABLE`, `labels` and `OBS`)
  \item drop all rows related to Switzerland (CH)
  \item for France use data for France metropolitain (FX) which excludes overseas territories
  \item calculate an additional variable `PC` that contains the percentages of land use classes per country
\end{itemize}

The package `dplyr`[^dplyr] provides a convenient syntax for carrying out these steps. Some of the label names will be changed to minimize the space taken up by the legend of the plot.
...
An initial version of the plot can be created with the following three lines of code. The `ggplot` package takes several default settings which are very not suitable for the present layout and we will change them in the next sections. However, two things are worth pointing out here:
\begin{itemize}
...
```{r firstplot,fig.cap="An initial version of the plot.",fig.width=5,fig.height=5,warning=FALSE,tidy=FALSE}
...
```{r windowdressing,fig.cap="A second version of the plot with changes made to several theme variables.",fig.width=6,fig.height=6,message=FALSE,warning=FALSE,tidy=FALSE}
